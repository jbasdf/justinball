...
const stopWords = ["a","able","about","above","abst","accordance","according","accordingly","across","act","actually","added","adj",
...
function parseImages($, url) {
  return $('img').map((i, img) => {
    ...
    if (url && imageUrl) {
      return URL.resolve(url, imageUrl);
    }
    return null;
  }).get();
}
...
function frequentWords($) {
  const pattern = /\w+/g;
  const importantTags = ['title', 'h1', 'h2', 'h3', 'h4', 'h5', 'strong', '#description'];
  let importantWords = '';
  const addWords = (j, tag) => {
    importantWords += ` ${$(tag).text()}`;
  };
  for (let i = 0; i < importantTags.length; i += 1) {
    $(importantTags[i]).each(addWords);
  }
  const matches = importantWords.match(pattern);
  const stats = {};
  if (matches) {
